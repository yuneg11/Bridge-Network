...
def train_step(batch, model, teacher_models, optimizer, kd_scale, kd_temp):
    images, labels = batch

    with torch.no_grad():
        teacher_logits = [
            teacher_model(images)["logits"]
            for teacher_model in teacher_models
        ]

    logits = model(images)["logits"]

    loss_ce = F.cross_entropy(input=logits, target=labels)
    loss_kd_list = [
        F.kl_div(
            F.log_softmax(input=logits / kd_temp, dim=1),
            F.log_softmax(input=teacher_logits / kd_temp, dim=1),
            log_target=True, reduction="batchmean",
        ) for teacher_logits in teacher_logits
    ]
    loss_kd = sum(loss_kd_list) / len(loss_kd_list)

    loss = (1 - kd_scale) * loss_ce + 2 * kd_scale * (kd_temp ** 2) * loss_kd
    ...
    return {"loss": loss.detach(), "loss_ce": loss_ce.detach(), "loss_kd": loss_kd.detach(), "acc1": acc1, "acc5": acc5}
...
def train_epoch(ctx, model, teacher_models, optimizer, scheduler, train_dataloader, device):
    model.train()

    if ctx["is_distributed"]:
        (train_dataloader._loader if is_xla else train_dataloader).sampler.set_epoch(ctx["epoch_idx"])

    meter = dev_utils.AverageMeter("loss", "loss_ce", "loss_kd", "acc1", "acc5")

    p, logging_batches, nb_fmt, kd_scale, kd_temp = \
        ctx["progress"], ctx["logging_batches"], ctx["nb_fmt"], ctx["kd_scale"], ctx["kd_temp"]
    ...
    for batch_idx, (images, labels) in enumerate(p.track(train_dataloader, description="Train", remove=True), start=1):
        ...
        metrics = train_step(
            batch=batch, model=model, teacher_models=teacher_models, optimizer=optimizer,
            kd_scale=kd_scale, kd_temp=kd_temp,
        )
        ...
        if batch_idx in logging_batches:
            logging.info(
                ...
                f"Loss CE {metrics['loss_ce']:.5f} ({meter.loss_ce:.5f})  "
                f"Loss KD {metrics['loss_kd']:.5f} ({meter.loss_kd:.5f})  "
...
def main(cfg, output_dir):
    ...
    for teacher in cfg.MODEL.KD.TEACHERS:
        if teacher.endswith(".pt"):
            base_path = ".".join(teacher.split("/")[:-1])
            filename = teacher.split("/")[-1]
        else:
            base_path = teacher
            filename = "best_acc1.pt"

        teacher_cfg = load_config(os.path.join(base_path, "config.yaml"))
        teacher_ckpt = torch.load(os.path.join(base_path, filename), map_location="cpu")

        teacher_model = build_model(teacher_cfg)
        teacher_model.load_state_dict(teacher_ckpt)
        teacher_model.eval()
        teacher_model.to(device)
        teacher_model = create_ddp_model(teacher_model)

        teacher_models.append(teacher_model)

    logging.debug("Teacher model:")
    for line in str(teacher_models[0]).split("\n"):
        logging.debug(line)
    ...
    with Progress(disable=(not is_master), speed_estimate_period=600, refresh_per_second=1) as progress:
        for epoch_idx in progress.trange(1, cfg.SOLVER.NUM_EPOCHS + 1, description="Epoch"):
            ...
            ctx = dict(
                ...
                kd_scale=cfg.MODEL.KD.SCALE, kd_temp=cfg.MODEL.KD.TEMPERATURE,
            )

            train_meter = train_epoch(ctx, model, teacher_models, optimizer, scheduler, train_dataloader, device)
            ...
            valid_meter = valid_epoch(dict(progress=progress), model, valid_dataloader, device)
...
if __name__ == "__main__":
    ...
    add_config_arguments(parser, cfg, aliases={
        ...
        "MODEL.KD.TEACHERS": ["-t", "--teachers"],
        "MODEL.KD.TEMPERATURE": ["-kdt", "--temperature"],
        "MODEL.KD.SCALE": ["-kds", "--scale"],
    })
