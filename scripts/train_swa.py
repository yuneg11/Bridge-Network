...
from bridge_net.solvers import build_optimizer, build_scheduler, SWA


def train_step(batch, model, optimizer, sampling=False):
    images, labels = batch
    ...
    optimizer.zero_grad()
    loss.backward()
    optimizer_step(optimizer, sampling=sampling)
...
def train_epoch(ctx, model, optimizer, scheduler, train_dataloader, device):
    ...
    p, logging_batches, nb_fmt, swa_epoch = (ctx["progress"], ctx["logging_batches"], ctx["nb_fmt"], ctx["swa_epoch"])
    ...
    for batch_idx, (images, labels) in enumerate(p.track(train_dataloader, description="Train", remove=True), start=1):
        ...
        metrics = train_step(batch=batch, model=model, optimizer=optimizer, sampling=(swa_epoch and batch_idx == num_batch))
...
def main(cfg, output_dir):
    ...
    with Progress(disable=(not is_master), speed_estimate_period=600, refresh_per_second=1) as progress:
        for epoch_idx in progress.trange(1, cfg.SOLVER.NUM_EPOCHS + 1, description="Epoch"):
            ...
            if epoch_idx == cfg.SOLVER.SWA_START_EPOCH:
                logging.info(f"[Epoch {epoch_idx:{ne_fmt}}/{cfg.SOLVER.NUM_EPOCHS:{ne_fmt}}] SWA start")
            ...
            ctx = dict(
                ...
                swa_epoch=(epoch_idx >= cfg.SOLVER.SWA_START_EPOCH),
            )
            ...
            # for evaluation with SWA
            if epoch_idx >= cfg.SOLVER.SWA_START_EPOCH:
                optimizer.load_swa_buffer()
                synchronize("load_swa_buffer")
            ...
            # for evaluation with SWA
            if epoch_idx >= cfg.SOLVER.SWA_START_EPOCH:
                optimizer.load_sgd_buffer()
                synchronize("load_sgd_buffer")
...
if __name__ == "__main__":
    ...
    add_config_arguments(parser, cfg, aliases={
        ...
        "SOLVER.SWA_START_EPOCH": ["-se", "--swa-start-epoch"],
