...
@torch.no_grad()
def repetition(images, labels, input_repeat: int, batch_repeat: int, sample_rho: float = 0.0):
    """
    sample_rho = 1 means that all images are simply repeated.
    """

    images = images.repeat(batch_repeat, 1, 1, 1)
    labels = labels.repeat(batch_repeat)

    shape, device = images.size(), images.device
    index = torch.stack([
        torch.arange(0, shape[0], device=device)
    ] + [
        torch.randperm(shape[0], device=device) for _ in range(input_repeat - 1)
    ], axis=1)

    perm_images = torch.reshape(images[index], (shape[0], input_repeat * shape[1], shape[2], shape[3]))
    perm_labels = labels[index]

    repeat_mask = (torch.rand(shape[0], device=device) < sample_rho)
    repeat_images = images.repeat(1, input_repeat, 1, 1)
    repeat_labels = labels[:, None].repeat(1, input_repeat)

    mimo_images = torch.where(repeat_mask[:, None, None, None], repeat_images, perm_images)
    mimo_labels = torch.where(repeat_mask[:, None], repeat_labels, perm_labels)

    return mimo_images, mimo_labels


def train_step(batch, model, optimizer, ensemble_size, batch_repeat, sample_rho):
    images, labels = batch
    images, labels = repetition(images, labels, ensemble_size, batch_repeat, sample_rho)
...
def valid_step(batch, model, ensemble_size):
    images, labels = batch
    images = images.repeat(1, ensemble_size, 1, 1)
    log_confs = model(images)["log_confidences"]
    log_confs = torch.logsumexp(log_confs, dim=1) - math.log(log_confs.size(1))
    ...
    loss = F.nll_loss(input=log_confs, target=labels, reduction="mean")
    acc1 = F.accuracy(input=log_confs, target=labels, topk=1)
    acc5 = F.accuracy(input=log_confs, target=labels, topk=5)
...
def train_epoch(ctx, model, optimizer, scheduler, train_dataloader, device):
    ...
    p, logging_batches, nb_fmt, batch_repeat, ensemble_size, sample_rho = \
        ctx["progress"], ctx["logging_batches"], ctx["nb_fmt"], ctx["batch_repeat"], ctx["ensemble_size"], ctx["sample_rho"]
    ...
    for batch_idx, (images, labels) in enumerate(p.track(train_dataloader, description="Train", remove=True), start=1):
        ...
        metrics = train_step(
            batch=batch, model=model, optimizer=optimizer, batch_repeat=batch_repeat,
            ensemble_size=ensemble_size, sample_rho=sample_rho,
        )
...
def valid_epoch(ctx, model, valid_dataloader, device):
    ...
    for images, labels in progress.track(valid_dataloader, description="Valid", remove=True):
        batch = (images.to(device), labels.to(device))
        metrics = valid_step(batch=batch, model=model, ensemble_size=ensemble_size)
        meter.update(metrics, n=len(batch[0]))
...
def main(cfg, output_dir):
    ...
    batch_repeat = cfg.MODEL.MIMO.BATCH_REPEAT
    sample_rho = cfg.MODEL.MIMO.SAMPLE_RHO
    ...
    with Progress(disable=(not is_master), speed_estimate_period=600, refresh_per_second=1) as progress:
        for epoch_idx in progress.trange(1, cfg.SOLVER.NUM_EPOCHS + 1, description="Epoch"):
            ...
            ctx = dict(
                ...
                ensemble_size=ensemble_size, sample_rho=sample_rho, batch_repeat=batch_repeat,
            )
...
if __name__ == "__main__":
    ...
    add_config_arguments(parser, cfg, aliases={
        ...
        "MODEL.MIMO.ENSEMBLE_SIZE": ["-es", "--ensemble-size"],
        "MODEL.MIMO.SAMPLE_RHO": ["-sr", "--sample-rho"],
    })
