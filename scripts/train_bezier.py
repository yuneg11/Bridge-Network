...
from bridge_net.bn_utils import is_bn, update_bn_stats


def train_step(batch, model, optimizer):
    images, labels = batch
    logits = model(images, bezier_lambda=torch.rand(()))["logits"]
...
def valid_step(batch, model):
    images, labels = batch
    logits = model(images, bezier_lambda=0.5)["logits"]
...
def train_epoch(ctx, model, optimizer, scheduler, train_dataloader, device):
    ...
    p, logging_batches, nb_fmt = (ctx["progress"], ctx["logging_batches"], ctx["nb_fmt"])
...
def main(cfg, output_dir):
    ...
    ckpt2 = torch.load(cfg.MODEL.CHECKPOINT_B, map_location="cpu")

    for k in state_dict:
        if k.endswith(".0"):
            state_dict[k] = ckpt0[k[:-2]]
        elif k.endswith(".2"):
            state_dict[k] = ckpt2[k[:-2]]
    ...
    has_bn = any([is_bn(m) for m in model.modules()])
    if has_bn:
        logging.info("Model has BatchNorm layers.")

        if cfg.DATASETS.NAME == "ImageNet1k":
            logging.info("Loading ImageNet1k alignment dataloader for BatchNorm layers.")
            align_dataloader = dataloaders["aln_loader"]

            if is_xla:
                align_dataloader = pl.MpDeviceLoader(align_dataloader, device)
        else:
            align_dataloader = train_dataloader
    ...
    with Progress(disable=(not is_master), speed_estimate_period=600, refresh_per_second=1) as progress:
        for epoch_idx in progress.trange(1, cfg.SOLVER.NUM_EPOCHS + 1, description="Epoch"):
            ...
            if has_bn:
                update_bn_stats(align_dataloader, model, device, bezier_lambda=0.5)
